...
class WhatsAppAutomation:
    ...
    def send_message_to_contact(self, phone_number):
        """Отправка сообщения и PDF файла контакту с улучшенной обработкой ошибок"""
        try:
            # Формируем URL для открытия чата с контактом
            ...
            pyautogui.click(x=1493, y=44)  # Клик по полю ввода сообщения (нужно настроить)
            time.sleep(1)
            pyautogui.write(self.message_text)
            time.sleep(1)
            ...
            # Отправляем PDF файл - метод 1: через скрепку
            try:
                # Нажимаем на кнопку прикрепления файлов (скрепка)
                attachment_button_x, attachment_button_y = self.coordinates.get('attachment_button', (640, 690))
                pyautogui.click(x=attachment_button_x, y=attachment_button_y)
                logging.info("Нажата кнопка прикрепления")
                time.sleep(2)

                # Нажимаем на опцию "Документ"
                document_option_x, document_option_y = self.coordinates.get('document_option', (640, 580))
                pyautogui.click(x=document_option_x, y=document_option_y)
                logging.info("Выбрана опция 'Документ'")
                time.sleep(3)

                # Вводим путь к файлу в диалог выбора файла
                pyautogui.write(self.pdf_path)
                time.sleep(2)
                pyautogui.press('enter')
                time.sleep(3)

                # Нажимаем на кнопку отправки файла
                send_button_x, send_button_y = self.coordinates.get('send_button', (850, 550))
                pyautogui.click(x=send_button_x, y=send_button_y)
                logging.info(f"Отправлен PDF файл для {phone_number}")
                time.sleep(5)

            except Exception as e:
                # Если метод 1 не сработал, пробуем метод 2: через drag-and-drop
                logging.warning(f"Не удалось отправить PDF через меню. Пробуем альтернативный метод: {e}")

                # Открываем проводник с файлом
                os.system(f'explorer /select,"{self.pdf_path}"')
                time.sleep(3)

                # Получаем координаты окна чата и перетаскиваем файл
                chat_window_x, chat_window_y = self.coordinates.get('chat_window', (500, 500))

                # Копируем файл в буфер обмена (Ctrl+C)
                pyautogui.hotkey('ctrl', 'c')
                time.sleep(1)

                # Переключаемся на окно браузера и вставляем (Ctrl+V)
                pyautogui.click(x=chat_window_x, y=chat_window_y)
                time.sleep(1)
                pyautogui.hotkey('ctrl', 'v')
                time.sleep(3)

                # Отправляем файл нажатием Enter
                pyautogui.press('enter')
                logging.info(f"Отправлен PDF файл альтернативным методом для {phone_number}")
                time.sleep(5)
